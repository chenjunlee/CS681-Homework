...
public class MainClient {
	...
	public static void main(String[] args) {
		...
		float minDiff = usedCars.stream().parallel().map( (Car usedCar)-> usedCar.getDomination(targetCar) )
											.reduce(500000, (result, carDifference)->{
															if(result==500000) return carDifference;
															else if(carDifference < result) return carDifference;
															else return result;} );
		System.out.println("Car difference is calculated by 50%price + 50%mileage + 600*year by using parallel stream");
